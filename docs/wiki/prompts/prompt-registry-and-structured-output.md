...
`director.risk.assessment@v2` 是自动导演异常评分与质量处置的唯一产品级入口。它接收已结构化的问题、审校结果、重规划建议和任务上下文，返回 1–8 分、类别、影响范围、证据、建议和 `canPause`。小问题建议局部修复，疑似误判建议记录质量债，计划窗口失配建议重规划；只有正文不可用、严重事实/数据冲突、受保护内容风险或连续失败可以暂停。

运行时只负责确定性安全后处理：将 AI 建议映射为 `light_repair_and_continue`、`record_debt_and_continue`、`replan_adjacent_and_continue` 或 `pause_for_manual`，并使用重试预算与熔断保证连续失败最终暂停。不要在服务层以关键词或正则重新猜测语义。
