...
- 任务中心不能只展示自动导演的失败诊断。对有结构化恢复动作的失败任务，必须在阻塞原因附近提升最低风险的 mutation 为“让 AI 处理并继续”；如果恢复目标的结构化阶段为 `pipeline`，同时提供定位到当前章节的“打开质量修复”。恢复必须复用导演跟进的幂等执行器，不能在前端另造一条重试分支。
...
简易创作是 `full_book_autopilot` 的用户入口，不是另一套正文生成器。它复用自动导演、章节 Runtime、质量债、事件账本和模型路由，只增加一份冻结的无人值守预算与恢复契约。

### Current Rule

- 启动前必须后端预检。预检根据剩余章节、自动审校/修复开关、历史平均调用时长与 Token 计算下一批预算；模型路由或调用预算阻断时，服务端也拒绝启动，不只依赖前端按钮。
- 默认每批最多 5 章、每章 12 次 AI 调用、每批 60 次调用和 120 分钟。这些是安全上限，不是必须用满的配额；达到上限时先完成当前章节的安全保存，再暂停或切换到下一批。
- 无人值守只对 408/409/429/5xx、连接重置、超时和结构化传输错误使用有界重试。合同缺失、语义门禁、受保护正文或数据完整性失败不得通过普通重试掩盖。
- 恢复必须产物优先。范围内已有未发布草稿时，即使生成阶段的章节合同不完整，也从审校/修复阶段读取该草稿；只有没有可用正文时才回到生成前置合同。
- AI 实况同时消费运行阶段与模型调用事件。命令返回 `executed` 只能向用户表达“已提交”；节点完成、失败或取消才是最终运行结果。
- 手机阅读格式的确定性检查只生成低风险建议，不单独触发修复或暂停。情绪钩子继续属于 `readerExperience` 和 `craftPlan` 的 AI 结构化职责，确定性规则不猜测剧情意图。

## 统一自动恢复入口

普通用户只执行 `auto_resolve_and_continue`，界面文案统一为“AI 自动处理并继续”。运行时根据当前任务状态、检查点和校验结果选择实际动作：安全回填/对账后继续、质量重规划后继续、原任务模型重试，或从最近安全检查点恢复。旧动作码只作为 API/历史通道兼容，不再并列为普通界面选项。

模型处理边界必须保守：只有明确的密钥/权限、余额/额度、429 限流或模型不可用才返回 `model_attention_required`。不带鉴权语义的 403 HTML 页、503、连接失败和结构化传输错误仍属于可自动恢复的普通运行异常。无可用正文、受保护内容或数据完整性风险仍不可自动放行。

任务中心、导演跟进、小说执行抽屉、批量操作和通道回调必须调用同一执行器。技术日志、检查点、质量债和旧兼容动作只放在高级详情或运维 API，不能在主处理区重新制造多路恢复选择。
