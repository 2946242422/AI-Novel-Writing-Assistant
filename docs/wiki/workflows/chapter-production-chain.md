...
运行时异常只在超时、429、5xx、连接重置或结构化传输错误时自动重试，无人值守默认最多一轮。章节目标、边界合同或业务门禁错误不做普通重试。范围内已有可用正文时，恢复从该草稿的审校/修复阶段继续，不因生成前置合同缺失重写正文；只有没有可用正文时，才由 JIT 规划补齐合同后进入写作。

本地速读检查只识别超短段落连发、文字墙、纯对白追溯和重复句首等可确定的排版风险，输出 `low` 级建议，不替代 AI 语义审校、不单独阻断、不因此增加模型调用。
