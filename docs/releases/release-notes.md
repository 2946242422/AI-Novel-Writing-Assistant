...
### 2026-08-18（重规划检查点恢复入口）

- 导演任务即使外层状态是“执行失败”，只要真实检查点为“需要处理重规划 / 质量建议”，导演跟进就会优先显示恢复与质量修复动作，不再将它错分为普通模型重试。
- 新增“跳过本次质量建议，从最近进度恢复”动作。它会保留已生成正文，将当前质量问题记入质量债，然后继续自动执行后续章节。
- 执行详情抽屉会把恢复动作和“打开质量修复”提升到底部固定操作区，不再让用户翻过里程碑、Token 统计和模型信息后才能处理。
- 恢复属于中风险动作，点击后会先明确告知“正文保留、问题记为质量债”，需要用户再次确认；打开质量修复只进行页面导航，不会立即改写正文。
- 导演跟进、执行详情和统一动作校验共用同一份语义，“失败 + 需重规划”任务可以从站内安全恢复，已有正文、检查点与模型绑定无需迁移。
