...
### 2026-08-18（AI 自动质量处置、无人值守与恢复入口）

- 全书自动执行新增统一 AI 质量处置结论：小问题选择轻修后继续，审校已通过或只有表达模式告警的疑似误判记录为质量债后继续，章节目标与计划窗口失配时自动重规划相邻章节后继续。
- “简易创作”底层作为无人值守生产执行：启动前会根据剩余章节、已有任务用量和冻结模型计算下一批的预计 AI 调用、Token 与时长，有阻断时不启动。
- 无人值守默认每批最多 5 章、每章最多 12 次模型调用、每批最多 60 次调用与 120 分钟，正文生成最多请求 8192 个输出 Token；超出预算时在当前章节安全保存后停止。
- 恢复改为产物优先：范围内已有草稿时，从审校、修复或结算所需阶段继续，不再先要求正文生成所需的完整章节合同。没有草稿时仍保留原有前置门禁。
- 自动重试只处理超时、429、5xx、连接重置等瞬时传输错误；章节目标、边界合同或数据校验失败直接进入修复或可恢复停顿，不再空转重试。
- AI 实况同时记录自动导演运行阶段和模型流；即使前置门禁在模型调用前失败，也会保留实际失败记录。恢复动作的提示改为“命令已提交”，不把命令受理冒充为任务完成。
- 手机阅读的超短段落连发、文字墙、纯对白追溯和重复句首由本地检查生成低风险提醒，不增加模型调用，也不单独阻断全书。读者体验合同同时约束进章问题、阻力蓄压、主角选择、关键转折、情绪余震和离章问题，并保留有净变化的安静章。
- `director.risk.assessment@v2` 会结合结构化审校、重规划结论、正文可用性和已有质量债给出处置；运行时只负责安全映射和预算熔断，不用关键词重新猜测。
- 没有可用正文、严重事实/数据冲突、受保护正文风险会直接暂停；同一章节窗口自动重规划后仍重复失配时触发熔断，避免无限重规划与 Token 消耗。
- 运行记录、导演跟进和小说执行抽屉的失败操作收敛为唯一的“AI 自动处理并继续”，不再要求用户在恢复、重试、修复和重规划之间作技术选择。
- 统一动作根据当前任务事实选择安全校验修复、结构化大纲回填、质量重规划继续、原模型重试或检查点恢复；批量处理时每个任务独立判断。
- 密钥/权限、余额/额度、模型限流和明确的模型不可用会返回简短可读的模型处理提示，不会继续空转。普通 403 HTML 错误页、503 和结构化传输错误不会被误判为 API Key 问题。
- 已有可用正文时，同类质量修复循环耗尽改为登记质量债并继续。技术日志、审校原因、检查点、Token、步骤和里程碑默认收入“高级详情”，超长日志可就地展开和收起。
- 钉钉和企业微信回调同步支持统一自动处理动作；旧的继续/重试动作码仍保留后端兼容，现行界面不再把它们并列给普通用户。
- 显式 `skip_quality_repair` 继续作为底层兼容命令保留；原有任务、正文、检查点、质量债和模型绑定无需迁移。

### 2026-08-17（模型接入与长日志展示优化）

- 快捷模型配置中的“自定义兼容接口”可以稳定保持选中，并继续填写厂商名称、API 地址、API Key 和模型信息。
- 已配置的内置厂商不会在用户选择自定义接口后重新覆盖当前选择。
- 填写自定义 OpenAI 兼容接口时，只输入 `https://api.example.com` 这样的域名即可；系统会自动补全为 `https://api.example.com/v1`。
- 配置表单会实时预览最终的 `…/v1/chat/completions` 请求地址，便于在检测前发现接口路径错误。
- 获取模型列表、连接检测、快捷配置和正式保存共用同一条地址规范化规则；已有 `/v1`、`/openai/v1` 等明确路径会保持原样。
- 模型厂商列表新增“本地 Codex”。已安装并登录 Codex 的用户可以直接复用 ChatGPT/Codex 账户完成规划、正文和结构化任务，不需要填写 OpenAI API Key。
- 本地 Codex 每次生成都使用独立的只读临时会话，桥接端口只监听本机；小说项目文件、Codex 登录令牌和 API Key 不会交给模型调用进程。
- JSON Schema 会直接交给 Codex 约束最终结果，生成结果仍会进入现有结构校验与修复流程；普通文本和结构化连接均可在模型设置中检测。
- 本地 Codex 默认单并发，调用会使用 ChatGPT 套餐中的 Codex 额度，适合个人本地生产，不等同于无限免费模型 API。
- 导演跟进列表对超长失败响应只显示四行预览，避免 HTML 错误页或网关日志撑满整个页面；选中记录后，失败原因、下一步建议、检查点摘要、校验原因和里程碑日志都会自动收起过长内容。
- “展开完整日志 / 收起日志”按钮始终位于内容上方，展开后无需滚动到日志末尾即可收起；按钮保留明确焦点和展开状态，可用键盘操作，普通短提示仍直接显示。
- 初次打开快捷配置时仍会自动推荐已经配置的厂商；原有内置厂商配置和已经保存的自定义厂商无需迁移。

### 2026-08-14（Gemini 本地代理与配置检测超时）

- Windows 本地使用 Gemini 官方接口时，服务端会自动通过 `http://127.0.0.1:7897` 发起普通文本、结构化输出及后续创作请求，不需要在快捷配置窗口额外填写代理地址。
- 本地代理端口或地址不同时，可以使用 `GEMINI_PROXY_URL` 覆盖；需要让所有 OpenAI 兼容模型请求共用代理时，可以使用 `AI_NOVEL_PROXY_URL`，同时兼容常见的 `HTTPS_PROXY`、`ALL_PROXY` 和 `HTTP_PROXY` 环境变量。
- 普通文本与结构化输出检测均增加 30 秒强制超时。代理未启动、网络不可达或服务端没有响应时，快捷配置会返回可见错误，不再无限停留在“正在检测”。
- Gemini 的本地默认代理不会强制作用于 DeepSeek 等其他供应商；未配置通用代理环境变量时，其他供应商继续按原方式直连。
- 现有 API Key、模型路由和旧项目数据无需迁移。

### 2026-08-12（AI 自主选择章节写法）

- 章节执行合同新增 AI 自主写法方案。AI 会综合本章任务、读者体验、人物关系、场景阻力、题材气质和近期重复风险，决定本章不额外指定技法、聚焦一种技法，或组合两到三种技法。
- 可选技法覆盖潜台词对话、权力关系变化、证据链调查、感官锚点、动作节奏、心理外化、环境施压、信息差悬念、误导与揭示、情绪递进、恐怖留白、喜剧节奏、空间调度、具体行动钩子和意象回环。
- 每项技法必须绑定具体场景，并说明叙事目的、执行指导和强度；写法方案不能增加新的剧情义务，章节任务、人物硬事实和章节边界始终优先。
- 正文写作、接收检查、章节审校和修复会共用同一份写法方案。审核只检查已选技法是否自然服务目标以及是否出现重点避免风险，不会要求补齐未选技法。
- 规划与审核会主动拦截固定感官数量、强行通感、比喻配额和解释性对白等机械写法，保留按章节实际需要选择技巧的空间。
- 章节执行页新增“AI 自动写法方案”，可查看选择依据、节奏策略、结尾策略、目标场景和重点避免项。
- 已有章节无需迁移数据库；没有保存写法方案的旧章节会自动使用自然执行模式，并继续兼容原有正文、审校和修复流程。
