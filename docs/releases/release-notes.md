...
- “失败 + 需重规划”任务的普通恢复动作收敛为“让 AI 处理并继续”，点击后重新进入同一份 AI 质量处置链，不要求用户选择修文、记债或重规划。
- 运行记录会在失败导演任务的阻塞原因旁直接提供“让 AI 处理并继续”和“打开质量修复”；前者沿用原任务模型与最近检查点，后者定位到当前问题章节。
- 执行详情优先展示“AI 轻修后继续 / 记录建议后继续 / 调整相邻章节后继续 / 需要确认”结论。审校原因、质量债、检查点、模型重试、Token、步骤和里程碑默认收入“高级详情”。
- 导演跟进同样优先展示推荐动作，检查点、审校原因、模型、里程碑和通道记录默认折叠。
