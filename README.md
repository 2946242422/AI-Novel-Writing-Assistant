...
- 全书自动执行遇到质量问题时，AI 会自动在轻修后继续、记录质量债后继续、重规划相邻章节后继续之间选择。
- 简易创作提供正式的无人值守生产：启动前显示下一批章节、AI 调用、Token 和时长预算，默认每批最多推进 5 章。
- 已保存草稿会从审校或修复阶段继续，不再因章节合同缺失重写正文；只有瞬时网络或服务错误才会自动重试。
- AI 实况同时展示自动导演阶段和真实模型调用；恢复按钮只提示“命令已提交”，最终成功或失败以运行事件为准。
- 只有没有可用正文、严重事实/数据冲突、受保护正文风险或同类自动处理连续失败时，才暂停询问用户。
- 失败处理收敛为唯一的“AI 自动处理并继续”：系统会根据任务状态自动选择安全修复、相邻章节重规划、检查点恢复或重试。
- 只有密钥/权限、余额/额度、模型限流或模型不可用会明确要求处理模型设置；普通 403 HTML 错误页、503 和结构化传输异常仍交给自动恢复。
- 有可用正文时，同类审校修复耗尽也会记录质量债并继续；技术日志、审校原因、质量债、检查点、Token 和里程碑默认收入可折叠的“高级详情”。
- 任务中心、导演跟进、小说执行抽屉以及钉钉/企微回调共用同一自动处理动作，批量恢复也会逐项独立判断。
- 原有任务、正文、检查点和已保存的模型绑定无需迁移。
