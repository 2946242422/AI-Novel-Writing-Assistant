...
- 普通恢复入口收敛为“让 AI 处理并继续”；审校原因、质量债、检查点、模型重试、Token 和里程碑默认收入“高级详情”。
- 运行记录中的失败导演任务可直接“让 AI 处理并继续”，也可打开定位到当前章节的质量修复页。
- “失败 + 需重规划”任务仍可从导演跟进和执行详情返回 AI 自动处置，不需要用户先理解检查点或模型重试。
