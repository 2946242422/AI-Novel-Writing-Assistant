...
- 导演任务即使被标记为“执行失败”，只要真实停点是需要处理质量建议，仍会显示正确的恢复操作，不再误导用户反复重试模型。
- 执行详情抽屉底部固定显示“跳过本次质量建议，从最近进度恢复”和“打开质量修复”，无需在里程碑、模型信息与日志之间翻找。
- 选择恢复时会先说明当前正文继续保留、本次问题记为质量债，确认后才会继续后续章节。
